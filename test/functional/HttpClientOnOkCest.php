...
class HttpClientOnOkCest
{
    public function dispatchEvent(FunctionalTester $I): void
    {
        ...
        $exitCode = $application->run(
            new StringInput('app:test:http-client-ok'),
            new BufferedOutput()
        );
        $I->assertEquals(0, $exitCode, 'Ожидали нулевой код выхода для команды');

        // Ожидаем минимум 2 спана: один для команды и один для HTTP запроса
        ...
        $I->assertNotNull($span, 'Ожидали найти спан для HTTP запроса');
